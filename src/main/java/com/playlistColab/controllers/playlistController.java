...
public class playlistController {
        ...
        @DeleteMapping(value = "/playlists/{PlaylistId}/songs/{songId}", produces = MediaType.APPLICATION_JSON_VALUE)
        public ResponseEntity<?> deleteSongFromPlaylist(@PathVariable long PlaylistId, @PathVariable String songId,
                        @AuthenticationPrincipal UserDetails userDetails) {
                log.info("deleting song with id {} from playlist with id {}", songId, PlaylistId);
                playlistService.deleteSongFromPlaylist(PlaylistId, songId, userDetails.getUsername());
                return ResponseEntity
                                .ok(new ApiResponse(true, "Song deleted successfully"));
        }
}
